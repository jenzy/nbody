...
void cpu( info_t *info ) {
	...
	float *vx = (float *) malloc( sizeof(float) * info->n );
	float *vy = (float *) malloc( sizeof(float) * info->n );
	float *vz = (float *) malloc( sizeof(float) * info->n );
	...
	for( s = 0; s < info->steps; s++ ) {
		for( i = 0; i < info->n; i++ ) { // za vsako telo i 
			...
			float dt2 = info->dt*info->dt;
			xnew[i] = x[i] + vx[i] * info->dt + 0.5f*ax*dt2; // nov polozaj za telo i
			ynew[i] = y[i] + vy[i] * info->dt + 0.5f*ay*dt2;
			znew[i] = z[i] + vz[i] * info->dt + 0.5f*az*dt2;
